...
start_chain() {
  ...
  nohup env RS_VERBOSE=${RS_VERBOSE:-1} python3 -m tools.cli chain >>"$CHAIN_LOG" 2>&1 & echo $! > "$PROJECT_ROOT/chain.pid"
}

start_trainer() {
  log "Launching trainer... (logs: $TRAIN_LOG)"
  nohup env RS_VERBOSE=${RS_VERBOSE:-1} python3 "$PROJECT_ROOT/train.py" >>"$TRAIN_LOG" 2>&1 & echo $! > "$PROJECT_ROOT/trainer.pid"
}
